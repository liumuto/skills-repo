...
把任意项目收敛到一套统一的规范骨架：`CLAUDE.md` + `docs/` 五区（plan / temp / architecture / tech / guide，每级带 README）+ 工具中立的 `.agents/`（skills 真相源）+ 由同步脚本生成的 `.claude/skills`、`.cursor/skills`。
...
AI 在被触发时，**先输出一份「骨架审计报告」**（逐项标 ✓ 已存在 / ✗ 缺失），经用户确认后，**只创建缺失的文件夹与文件，对任何已存在文件零覆盖**，最终运行同步脚本把 `.agents/skills/` 镜像到 `.claude/skills/` 与 `.cursor/skills/`。
...
├── .agents/
│   ├── skills/                    # ★ skills 唯一真相源
│   └── sync-skills.sh             # 同步脚本（→ .claude / .cursor）
├── .claude/
│   └── skills/                    # 由 sync-skills.sh 生成，勿手改
└── .cursor/
    ├── rules/   karpathy-guidelines.mdc   # ★ 独立维护，不属 skill 同步体系
    └── skills/                    # 由 sync-skills.sh 生成，勿手改
```
...
| 8 | 同步 skills | 拷入 `.agents/sync-skills.sh` 并运行，把 `.agents/skills/` 镜像到 `.claude/skills/`、`.cursor/skills/`（rules 不在同步范围） |
...
- **真相源唯一**：skills 真内容只在 `.agents/skills/`；`.claude/skills`、`.cursor/skills` 是生成物，会被同步脚本整体重建，勿手改。
...
- **Cursor rules 独立于同步体系**：`.cursor/rules/` 用 `*.mdc`（带 globs/alwaysApply），**不参与 skills 同步**。本 skill 仅为其铺底一份与 CLAUDE.md 一致的 `karpathy-guidelines.mdc`；其余规则由用户手动维护。
- **与相邻 skill 边界**：
